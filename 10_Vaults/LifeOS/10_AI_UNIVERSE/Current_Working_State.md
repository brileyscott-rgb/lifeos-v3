...
- **LifeOS Repo/MCP Radar V1 created (2026-07-08)**: Safe candidate tracking and evaluation system for external GitHub repos, MCP servers, Docker tools, n8n templates, and AI-agent frameworks. No external repos cloned, no packages installed, no services activated.
   - **Repo Radar** (`40_Services/docs/LifeOS_Repo_Radar.md`): 10 candidates evaluated across 16 dimensions (risk tier, secrets, Docker socket, filesystem, network, browser automation, shell execution, sandboxability, etc.). Each entry includes first safe test, rollback notes, reality-check findings, and recommendation (sandbox/defer/reject/reclassify/discovery-reference).
   - **MCP Catalog** (`40_Services/docs/LifeOS_MCP_Catalog.md`): 20 entries grouped into 5 categories — Approved for Sandbox Test (4), Deferred (7), High-Risk Quarantine (2), Discovery Indexes (2), Rejected for Now (5). Combines the 10 new Repo Radar candidates with the 10 existing MCP candidate catalog entries.
   - **Evaluation Checklist** (`40_Services/docs/LifeOS_External_Tool_Evaluation_Checklist.md`): 14-point checklist covering secrets, Docker socket, filesystem access, repo writes, external writes, network ports, browser automation, shell execution, tests, maintenance, license, read-only mode, sandboxability, and clean removal. Includes red-flag pattern detection.
   - **CLI Script** (`40_Services/scripts/lifeos_repo_radar.py`): Python stdlib-only read-only utility for exploring the candidate registry. Supports `--text`/`--json` output, filtering by risk tier, recommendation, and category, detail view, catalog group summary, and list commands. No network, no secrets, no file writes.
   - **Registry JSON** (`40_Services/scripts/lifeos_repo_radar_registry.json`): Structured candidate data with catalog groups cross-referencing the older MCP Candidate Catalog.
   - **Tests** (`40_Services/scripts/tests/test_lifeos_repo_radar.py`): 12 unit tests covering registry loading, filtering (exact, substring, case-insensitive, combined, no-match), text output formatting, default registry path, and read-only source enforcement (no secrets, no network, no subprocess, stdlib-only imports). All 45 script tests passing.
   - **Key findings**: 3 of 10 candidates are NOT MCP servers (OpenHands = agent platform, agent-sandbox = Kubernetes operator, MarkItDown = library → reclassified). Playwright MCP permanently rejected (vendor says "NOT a security boundary"). Qdrant MCP rejected for V1 (infrastructure doesn't exist). mcpo + Official MCP sub-servers approved for sandbox test.
   - **Safety preserved**: No external repos cloned. No packages installed. No Docker services started. No secrets exposed. No .env read. No canonical vault writes. All 436 tests passing across all suites.
...
- **Capture-to-Vault Orchestrator V0 implemented (2026-07-08)**: Full gated progression: capture → view → classify → specialist delegation → MCP-refined proposal → user confirmation/revision → controlled vault import.
  ...
  - **Tests**: 627 total (38 MCP + 104 orchestrator + 48 capture_processors + 227 Telegram + 103 Action API + 18 Status API + 45 scripts + 33 Capture API). Python stdlib only. Zero pip/npm deps.
  ...
  - **Integration stabilization** (2026-07-09): Fixed 3 integration blockers found during post-implementation review:
   ...
   - **First live `/kt latest` exercised by user (2026-07-09)**: Failed with argparse `unrecognized arguments: --output` error. Root cause: `--output` was only a global option on the parent parser; Telegram passed it after subcommand where subcommand parsers lacked it.
   - **Live Telegram fix** (2026-07-09): Added `--output` to all 5 subcommand parsers (propose-knowledge, view-proposal, reject-proposal, revise-proposal, approve-import). All three CLI positional forms now accepted (global before subcommand, after `--capture`, before `--capture`). Telegram error handling hardened — no raw stderr/stdout/tracebacks exposed to users. Input validation added to reject shell metacharacters in capture references. Proposal card format improved with explicit "Vault: unchanged" field.
   - **Tests**: 627 total (38 MCP + 104 orchestrator + 48 capture_processors + 227 Telegram + 103 Action API + 18 Status API + 45 scripts + 33 Capture API). Added 6 CLI positional tests + 6 Telegram /kt safety tests.
   - **Live validation status**: Telegram service restarted with fix code (22:59 CDT). CLI buffer-only proposal smoke test passed (proposal generated, importer dry-run correctly refused pending proposal). Buffer: 1 proposal file. Vault: 0 files changed. **Live user retest still required** — user must send `/kt latest` again to confirm the fix works in Telegram.
   - **Next user action**: Send `/kt latest` from @my_lifeOS_08bot. Expected: proposal card with Proposal ID, Capture ID, Classification, Destination, "Vault: unchanged", and confirmation commands.
