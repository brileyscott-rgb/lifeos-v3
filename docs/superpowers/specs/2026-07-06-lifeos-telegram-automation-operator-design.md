...
## 19. LifeOS Telegram Capture Intake Vision

This section defines the expanded capture intake model that the Telegram Operator will provide as the mobile capture front door for LifeOS.

### Capture Modes

**Immediate capture** — `/capture <text>`
- User sends `/capture` followed by text (e.g., `/capture Great article on Rust async patterns https://example.com`).
- System creates a pending capture immediately and replies with a receipt.
- No follow-up interaction required.

**Capture mode** — `/capture` with no text
- User sends `/capture` alone to enter an interactive capture mode.
- The bot replies: "Capture mode started. Send me your text, link, thought, idea, or note. Send /cancel to exit."
- Every subsequent message from the user (before /cancel or timeout) becomes the capture payload.
- The first message after entering capture mode creates a single capture and exits capture mode.
- `/cancel` exits capture mode without creating a capture.
- Capture mode times out after a configurable idle period (e.g., 10 minutes). On timeout, the bot sends "Capture mode timed out. Send /capture to start again."

### Initially Supported Payloads

- **Text** — plain text notes, thoughts, ideas, or reminders
- **Links** — URLs shared as free text (extracted in the capture itself)
- **Thoughts** — free-form ideas without structure
- **Ideas** — creative concepts or project notions
- **Notes** — quick memos or observations

### Later Supported Payloads (future scope, not implemented yet)

- **Photos** — sent as Telegram photo messages; metadata and file reference captured
- **Voice memos** — Telegram voice messages; transcript to be generated by future AI extraction
- **Documents/files** — sent as Telegram document attachments; file reference and metadata captured

### Capture Storage

Every capture lands in `pending_review/` first, regardless of payload type:
- `30_Capture/pending_review/`

Each capture includes standard frontmatter:
```yaml
---
...
created_at: "2026-07-06T18:06:06Z"
sender_id: 123456789
---
```
...
After a capture is created, the bot replies with a receipt message:
...
Existing commands (already defined in section 7):

| Command | Description |
|---|---|
| `/pending` | List pending captures with numbered index |
| `/view <number>` | View a specific capture's full content |
| `/approve <number>` | Approve a capture (moves to `approved/`) |
| `/reject <number>` | Reject a capture (moves to `rejected/`) |

All captures start in `pending_review` and require explicit approve/reject before they proceed to any later processing stage.

### Receipt and Traceability

Every capture operation produces an `event_id` returned to the user. These trace IDs link the Telegram interaction to the event log entry for debugging and audit trail purposes.

### Key Architecture Principle: Separation of Concerns

Keep capture, extraction, and file creation separated:

- **Raw capture intake**: Telegram → n8n → Action API → `30_Capture/pending_review/`
- **Review**: Telegram → n8n → Action API → approve/reject → `30_Capture/approved/` or `30_Capture/rejected/`
- **Future AI extraction**: Approved capture → AI extraction → draft/proposal
- **Future file creation**: Approved proposal → controlled LifeOS processor → vault/project/idea/knowledge files

Do not collapse these into one unsafe workflow.

---

## 20. Future AI Capture Processing Pipeline

> **This section describes future work. Do not implement any AI extraction or proposal pipeline until the raw capture and review flow is stable and approved.**

### Purpose

After a capture is approved, a future AI extraction pipeline can read the approved capture, extract structured information, and produce a proposal for LifeOS file creation.

### AI Extraction Scope

The AI pipeline reads an approved capture and extracts:

- **Title** — descriptive title derived from capture content
- **Summary** — concise one-to-three-sentence summary
- **Capture type** — classification: `link`, `idea`, `thought`, `note`, `action_item`, `reference`, `photo`, `voice_memo`
- **Source links** — extracted URLs with domain metadata
- **Key details** — important entities, dates, numbers, or references
- **Action items** — actionable tasks extracted from the capture, if any
- **Suggested LifeOS destination** — which LifeOS directory the content fits best (e.g., `10_Vaults/LifeOS/20_Ideas/`, `30_Projects/`, `Knowledge/`)
- **Proposed files to create or update** — specific file paths, content drafts, and frontmatter suggestions

### Proposal Model, Not Direct Writes

The AI produces a **proposal** — a structured document that describes what should be created or updated, without directly writing anything.
...
```json
{
  ...
  "summary": "Reference link to an article about Rust async patterns with recommendations for project use.",
  "capture_type": "link",
  ...
  "proposed_files": [
    {
      "path": "10_Vaults/LifeOS/30_Knowledge/Rust/async-patterns-reference.md",
...
| Event | When | Details |
|---|---|---|
| `ai.extraction_started` | AI begins reading a capture | `capture_id` |
| `ai.extraction_completed` | AI finished extraction | `capture_id`, `proposal_id` |
| `ai.proposal_approved` | User approved the proposal | `proposal_id` |
| `ai.proposal_rejected` | User rejected the proposal | `proposal_id` |
| `ai.file_creation_started` | Processor begins applying proposal | `proposal_id` |
| `ai.file_creation_completed` | Processor finished file creation | `proposal_id`, files created |

### Do Not Implement This Pipeline Yet

The AI extraction pipeline is future work. It must not be built until:

1. Raw capture and review flow is stable and tested
2. Capture mode, receipts, and review commands work end-to-end
3. A controlled Proposal Processor API is designed and approved
4. AI model integration (OpenAI, Claude, or local) is explicitly reviewed for security

---

## 21. Future Processor Boundary

> **This section describes the planned architecture for the controlled file creation step. Do not implement yet.**

### Responsibility Split

| Component | Responsibility |
|---|---|
| **Action API** | Capture lifecycle only — create, list, view, approve, reject. No vault file creation. |
| **Future Processor API** | Read approved proposals, apply validated file changes to LifeOS vault/directories. Template-based, validation-gated. |
| **AI Extractor** | Read-only: reads approved captures, produces proposal documents. No direct write authority. |
| **n8n** | Orchestrator only: routes data between Telegram, Action API, and future components. No arbitrary file writes. |

### Processor Requirements (future)
...
- n8n must not directly write arbitrary vault files
- AI must not receive shell, git, or direct file-write authority
- The processor must not have access to secrets, credentials, or Docker socket
- No component may write to the vault without going through the approval gate

---

## 22. Polished Telegram UX Ideas

> **This section lists planned UX improvements for the Telegram bot. These are design ideas, not implemented yet.**

### `/menu` Command
- Returns an inline keyboard menu with buttons for each major action
- Buttons: 📝 Capture, 📋 Pending, ✅ Approve, ❌ Reject, ℹ️ Status, ❓ Help
- Makes the bot feel like an app from a phone

### Telegram Command Menu
- BotFather `/setcommands` registration so Telegram shows available commands in the input field
- Users see `@LifeOSBot` command suggestions as they type

### Inline Buttons
- Approve/reject buttons on pending capture list items (where feasible via Telegram API)
- "View Details" button for each capture
- Quick action buttons after receipt: [View Pending] [Capture Again]

### Confirmation Cards
- Approval sends a confirmation card:
  ```
  ✅ Capture Approved

  capture_id: capture_20260706_article_link
  status: approved → pending_processing

  What's next?
  • This capture will enter the AI extraction pipeline when active
  ```
- Rejection sends:
  ```
  ❌ Capture Rejected
...
### Capture Receipts
- Rich receipt format with emoji indicators and structured fields
- `capture_id`, `event_id`, capture type, status, timestamp
- Next-available actions listed with command syntax

### Event/Trace IDs in Replies
- Every reply includes its `event_id` so users can reference it for support or debugging
- Trace IDs correlate Telegram messages with event log entries

### `/selftest` Command
- Runs a self-test: checks Action API reachability, capture directory access, event log writability
- Returns pass/fail per check
- Useful for deployment verification and troubleshooting

### `/last` Command
- Shows the last capture or action performed
- Quick way to re-check receipt or find an event_id after switching apps

### `/review` Mode
- Steps through pending captures one by one
- Shows full capture content, then offers inline buttons: Approve, Reject, Skip, Cancel
- Phone-friendly "swipe" equivalent for batch review

### Safe "Latest" Buttons
- For `/approve latest` and `/reject latest`, resolve to the most recent pending capture (highest index, newest)
- Bot confirms which capture was targeted: "Approved capture #3: 'Quick note about...'"

### Emergency Kill-Switch / Runbook
- A documented procedure to disable the Telegram bot quickly:
  1. `curl -X POST https://api.telegram.org/bot<TOKEN>/deleteWebhook`
  2. Deactivate n8n workflow from the UI
  3. If n8n unreachable, stop the container: `docker-compose stop n8n`
- Documented in the rollback checklist (section 17)
